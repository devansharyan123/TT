...
router.get("/timetable", async (req: Request, res: Response) => {
  try {
    ...
    const dateTasks = await prisma.task.findMany({
      where: {
        userId: user.id,
        sectionId: section.id,
        date: {
    ...
    const definitions = new Map<string, (typeof recurringTasks)[number]>();
    for (const task of recurringTasks) {
      if (task.weekday === null) continue;
      ...
      if (!definitions.has(key)) definitions.set(key, task);
    }

    const existingByDate = new Map<string, typeof dateTasks>();
    ...
    const missingOccurrences: Prisma.TaskCreateArgs["data"][] = [];
    for (const date of dates) {
      ...
      for (const [key, source] of definitions) {
        if (source.weekday !== (weekday + 6) % 7 || existingKeys.has(key)) continue;
    ...
    const created = missingOccurrences.length > 0
      ? await prisma.$transaction(
          missingOccurrences.map((data) => prisma.task.create({ data, include: { section: true } }))
        )
      : [];
