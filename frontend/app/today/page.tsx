...
function readStreakState(section: string): StreakState {
...
function writeStreakState(section: string, state: StreakState) {
  if (typeof window === "undefined") return;
  localStorage.setItem(getStreakStorageKey(section), JSON.stringify(state));
}
...
export default function TodayPage() {
  ...
  const evaluateStreakSignals = useCallback(async (todayTasks: Task[], section: string) => {
    const weekDates = getPastIsoDates(7);
    const weekResult = await Promise.allSettled(weekDates.map((d) => fetchTasks(d, section)));
    ...
    const currentState = readStreakState(section);
    if (currentState.history[todayISO]) {
      ...
      writeStreakState(section, updatedState);
  ...
  const loadTasks = useCallback(async () => {
    try {
      const data = await fetchTasks(todayISO, selectedSection);
      setTasks(sortByScheduledTime(data));
      await evaluateStreakSignals(data, selectedSection);
    } catch {
      setError("Could not connect to server — showing demo data.");
      const fallback = demoTasks().map((t) => ({ ...t, section: selectedSection }));
      setTasks(sortByScheduledTime(fallback));
      await evaluateStreakSignals(fallback, selectedSection);
    } finally {
      setIsLoading(false);
    }
  }, [evaluateStreakSignals, selectedSection]);
  ...
  const handleUpdate = async (id: string, updates: Partial<Task>) => {
    setTasks((prev) => {
      ...
      void evaluateStreakSignals(next, selectedSection);
      return next;
    });
    try {
